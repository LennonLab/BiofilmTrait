...
Project looking at bifilm production of *Pseudomonas aerugionosa* in soil environment

```{r}
rm(list=ls())
getwd()
setwd("/Users/lennonj/GitHub/Biofilms")
```
...
## SUPPLEMENTARY FIGURE 1: BIOFILM-LAG TIME TRADEOFF
...
```{r}
png(filename="/Users/lennonj/GitHub/Biofilms/figures/SupplementalFigure1.png",
...
#mtext(expression(paste(rho, "= -0.38"), line = -10, cex = 1, at = 70))
...
However, the relationship doens't look linear. 
...
## TESTING GROWTH RATES OF 'OE' AND 'NM' UNDER LAB CONDITIONS
...
```{r}
rate <- (log(data_growth$OD_t38) - log(data_growth$OD_t0)) / (data_growth$t38-data_growth$t0)
...
```{r}
# Set-up for printing 
png(filename="/Users/lennonj/GitHub/Biofilms/figures/Figure1.png",
...
# Make inital plot
otoole.plot <- plot(otoole.means, log = "y", ylim = c(0.05, 2.4), xlim = c(0.5, 2.5), 
      pch = 22, bg = c("white", "grey"), lwd = 2, cex = 3, yaxt = "n", xaxt = "n", 
      cex.lab = 2, cex.axis = 1.5, las = 1, ylab = "", xlab = "")
      
      box(lwd = 2)

# Add y-label axis for NM vs. OE plot
mtext(expression('Biofilm Production (a'[550]*')'), side = 2, outer = TRUE, cex = 1, 
      line = -4, adj = 0.87)
...
biofsurv.fit <- survfit(biofsurv ~ surv$Group, conf.int = TRUE, type = "kaplan-meier")
biofsurv.diff <- survdiff(Surv(surv$Days,surv$Event) ~ surv$Group, data = surv)
```

Plotting
```{r}
png(filename="/Users/lennonj/GitHub/Biofilms/figures/Figure2.png",
    width = 1200, height = 1200, res = 96*2)
...
## FIGURE 3: GROWTH RATES VS. WATER POTENTIAL
...
# Plotting
png(filename="/Users/lennonj/GitHub/Biofilms/figures/Figure3.png",
    width = 1200, height = 1200, res = 96*2)
par(mar = c(7, 7, 5, 7))
...
# Add legend
legend(-1.2, 3.24, c("OE", "NM"), pch = 22, pt.bg = c("grey", "white"), pt.cex = 2, 
       pt.lwd = 2, bty = 'n', y.intersp = 1, lty = c(6,4), lwd = 2.5)
...
## FIGURE 4: RESPIRATION VS. WATER POTENTIAL
...
fit1 <- mle2(R ~ dnorm(mean = a, sd = z), start = list(a = A, z = Z),data = resp) 
              # null
...
fit4 <- mle2(R ~ dnorm(mean = a*exp(-((abs(W + x))/b)^t), sd = z), method = "L-BFGS-B",
             lower = c(x = 0.0000, b = 0.001), start = list(a = A, x = X, b = B,
             t = T, z = Z), data = resp, parameters = c(x ~ Strain)) 
             # opt water differs between strain 

fit5 <- mle2(R ~ dnorm(mean = a*exp(-((abs(W + x))/b)^t), sd = z), method = "L-BFGS-B",
             lower = c(x = 0.0000, b = 0.001), start = list(a = A, x = X, b = B, 
             t = T, z = Z), data = resp, parameters = c(b ~ Strain)) 
             # breadth differs between strains

fit6 <- mle2(R ~ dnorm(mean = a*exp(-((abs(W + x))/b)^t), sd = z), method = "L-BFGS-B",
             lower = c(x = 0.0000, b = 0.001), start = list(a = A, x = X, b = B, 
              t = T, z = Z), data = resp, parameters = c(t ~ Strain)) 
              # tau differs between strains

fit7 <- mle2(R ~ dnorm(mean = a*exp(-((abs(W + x))/b)^t), sd = z), method = "L-BFGS-B",
             lower = c(x = 0.0000, b = 0.001), start = list(a = A, x = X, b = B,
             t = T, z = Z), data = resp, parameters = c(a ~ Strain, x ~ Strain)) 
             # max and opt differ between strains

fit8 <- mle2(R ~ dnorm(mean = a*exp(-((abs(W + x))/b)^t), sd = z), method = "L-BFGS-B",
             lower = c(x = 0.0000, b = 0.001), start = list(a = A, x = X, b = B,
             t = T, z = Z), data = resp, parameters = c(a ~ Strain, b ~ Strain)) 
             # max and breadth differ between strains

fit9 <- mle2(R ~ dnorm(mean = a*exp(-((abs(W + x))/b)^t), sd = z), method = "L-BFGS-B",
             lower = c(x = 0.0000, b = 0.001), start = list(a = A, x = X, b = B,
             t = T, z = Z), data = resp, parameters = c(a ~ Strain, t ~ Strain)) 
             # max and tau differ between strains

fit10 <- mle2(R ~ dnorm(mean = a*exp(-((abs(W + x))/b)^t), sd = z), method = "L-BFGS-B", 
              lower = c(x = 0.0000, b = 0.001), start = list(a = A, x = X, b = B, 
              t = T, z = Z), data = resp, parameters = c(x ~ Strain, b ~ Strain)) 
              # opt and breadth differ between strains

fit11 <- mle2(R ~ dnorm(mean = a*exp(-((abs(W + x))/b)^t), sd = z), method = "L-BFGS-B",
              lower = c(x = 0.0000, b = 0.001), start = list(a = A, x = X, b = B,
              t = T, z = Z), data = resp, parameters = c(x ~ Strain, t ~ Strain)) 
              # opt and tau differ between strains

fit12 <- mle2(R ~ dnorm(mean = a*exp(-((abs(W + x))/b)^t), sd = z), method = "L-BFGS-B",
              lower = c(x = 0.0000, b = 0.001), start = list(a = A, x = X, b = B,
              t = T, z = Z), data = resp, parameters = c(b ~ Strain, t ~ Strain)) 
              # breadth and tau differ between strains

fit13 <- mle2(R ~ dnorm(mean = a*exp(-((abs(W + x))/b)^t), sd = z), method = "L-BFGS-B",
              lower = c(x = 0.0000, b = 0.001), start = list(a = A, x = X, b = B, 
              t = T, z = Z), data = resp, parameters = c(a ~ Strain, x ~ Strain, 
              b ~ Strain)) 
              # max, opt, breadth differ between strains

fit14 <- mle2(R ~ dnorm(mean = a*exp(-((abs(W + x))/b)^t), sd = z), method = "L-BFGS-B",
              lower = c(x = 0.0000, b = 0.001), start = list(a = A, x = X, b = B, 
              t = T, z = Z), data = resp, parameters = c(a ~ Strain, x ~ Strain, 
              t ~ Strain)) 
              # max, opt, tau differ between strains

fit15 <- mle2(R ~ dnorm(mean= a*exp(-((abs(W + x))/b)^t), sd = z), method = "L-BFGS-B",
              lower = c(x = 0.0000, b = 0.001), start = list(a = A, x = X, b = B, 
              t = T, z = Z), data = resp, parameters = c(a ~ Strain, b ~ Strain, 
              t ~ Strain)) 
...
AIC.table <- AICtab(fit1, fit2, fit3, fit4, fit5, fit6, fit7, fit8, fit9, fit10,
              fit11, fit12, fit13, fit14, fit15, fit16, fit17, delta = T, weights = T)
AIC.names <- attributes(AIC.table)$row.names
AIC.table <- as.data.frame(do.call(cbind, c(AIC.table)))
AIC.table$names <- AIC.names
write.table(AIC.table, "~/GitHub/Biofilms/figures/AIC.table.txt", sep = "\t") 

# Conclusion from based on AIC is that the following models all perform equally
# well: fit10, fit 4, fit7, fit16, fit13, and fit11. 
# However, all of these models contain Wopt. Therefore, we use fit4, which only
...
```{r}
# Following calculated estimates and error for `Wopt`, optmimum water potential
NM.opt <- fit4@coef[2]
NM.opt.e <- 0.022994 # con't figure out how to extract Std. Error from fit4
OE.opt <- fit4@coef[3] + fit4@coef[2]
OE.opt.e <- NM.sig.e + 0.039542
```
...
png(filename="/Users/lennonj/GitHub/Biofilms/figures/Figure4.png",
    width = 1200, height = 1200, res = 96*2)
...
# First curve is for NM strain
# curve(coef(fit10)[[1]]*exp(-((abs(coef(fit10)[[2]]+x)/coef(fit10)[[4]]))^coef(fit10)[[6]]), 
...
# OEx=coef(fit10)[2]+coef(fit10)[3] # modifies optimum parameter relative to NM
#OEb=coef(fit10)[4]+coef(fit10)[5] # modified breadth parameter relative to NM
...
## FIGURE 5: REWETTING COLUMN EXPERIMENT
...
# Make main plot
```{r}
png(filename="/Users/lennonj/GitHub/Biofilms/figures/Figure5.png",
...
## FIGURE 6: SPECIES INTERACTIONS VIA TESTER STRAIN
...
# Main plot
png(filename="/Users/lennonj/GitHub/Biofilms/figures/Figure6.png",
    width = 1200, height = 1200, res = 96*2)
par(mar = c(7, 7, 5, 7))
...
# # Add multiple regression lines (not printed; instead using "global"
# parameters because strain parameters are not significant)
# clip(0.05,-1.6,-0.1,0.05)
# abline(a = tester.int, b = tester.slp, col = "black", lwd = 2.5, lty = 6)
# 
# clip(0.05,-1.4,-3.5,3)
# abline(a = tester.NM.int, b = tester.NM.slp, col = "red", lwd = 2.5, lty = 4)
# 
# clip(0.05,-1.3,-3.5,3)
# abline(a = tester.OE.int, b = tester.OE.slp, col = "blue", lwd = 2.5, lty = 4)
